...
def run():
    ...
    # Drop status rows for companies no longer in the config (e.g. moved to
    # watchlist) so their dot doesn't linger on the dashboard forever.
    current_names = [c["name"] for c in companies_cfg.get("ats_tracked", [])]
    cur.execute("SELECT company FROM source_status")
    existing_status_names = [row[0] for row in cur.fetchall()]
    stale_names = [n for n in existing_status_names if n not in current_names]
    for n in stale_names:
        cur.execute("DELETE FROM source_status WHERE company = ?", (n,))
    if stale_names:
        summary_lines.append(f"  [REMOVED] no longer tracked, dropped from dashboard: {', '.join(stale_names)}")
