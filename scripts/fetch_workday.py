...
Because it's unofficial:
  ...
  - some companies block non-browser User-Agents

Treat every Workday result as "best effort." When a company shows as broken
in the dashboard, the fastest fix is usually: open their real careers page
in a browser, open dev tools -> Network tab, filter for "cxs", and copy the
exact request URL you see -- then update wd_host/tenant/site in companies.yaml.
"""
import requests
...
def fetch_workday_jobs(tenant: str, site: str, wd_host: str = None) -> dict:
    ...
    for host in hosts_to_try:
        ...
        if resp.status_code != 200:
            last_error = f"HTTP {resp.status_code} on {host}"
            continue
        ...
        if postings is None:
            last_error = f"unexpected response shape on {host}"
            continue
